...
scene.add(box);
const label = new _default("check console", 0.3);
box.add(label);
label.position.y = 3;
...
controlsManager.listen(threeFreeformControls.EVENTS.DRAG_START, (_, handleName) => {
  orbitControls.enabled = false;
  console.log(`DRAG_START: ${handleName}`);
});
controlsManager.listen(threeFreeformControls.EVENTS.DRAG, (_, handleName) => {
  console.log(`DRAG: ${handleName}`);
});
controlsManager.listen(threeFreeformControls.EVENTS.DRAG_STOP, (_, handleName) => {
